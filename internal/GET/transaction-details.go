...
// TransactionDetails : GET, get tx by id
func TransactionDetails(writer http.ResponseWriter, request *http.Request) {
	...
	if request.Method != HTTP.GET {
		message := HTTP.NewInvalidMethodMessage(HTTP.GET, request)
